...
def _sms_beem(phone: str, message: str) -> bool:
    ...
    Setup (5 minutes, free registration):
      ...
      4. Set SMS_PROVIDER=beem in .env
         ...
         BEEM_SENDER_NAME=KUKU  (up to 11 chars — register with Beem first)
    """
    # .strip('"\' \t\r\n') guards against copy-paste artifacts in the Render
    # env var UI (quotes, trailing spaces, stray \r from Windows clipboard)
    # that would otherwise produce a source_addr Beem doesn't recognize.
    api_key    = getattr(settings, 'BEEM_API_KEY', '').strip('"\' \t\r\n')
    secret_key = getattr(settings, 'BEEM_SECRET_KEY', '').strip('"\' \t\r\n')
    sender     = (getattr(settings, 'BEEM_SENDER_NAME', '') or 'INFO').strip('"\' \t\r\n')
    ...
    masked_key = (api_key[:4] + '...' + api_key[-4:]) if len(api_key) > 8 else '(too short/empty)'
    print(
        f"[Beem] Sending → sender_id={sender!r} (len={len(sender)}) "
        f"api_key={masked_key} (len={len(api_key)}) dest={dest}"
    )
    try:
        resp = http_requests.post(
            "https://apisms.beem.africa/v1/send",
